...
Only uses: numpy, pandas, sys, csv, random, re, os (all stdlib/allowed).
...
# ── Load saved model parameters once at import time ───────────────────────────
...
# ── TF-IDF transform (pure python/numpy) ─────────────────────────────────────
_TOKEN_RE = re.compile(r"(?u)\b\w\w+\b")

def _tfidf_row(text, vocab, idf):
    """Transform a single string into a TF-IDF vector (numpy array)."""
...
def _gb_proba(X, feat, thresh, left, right, val, lr, init_raw):
    ...
    for k in range(n_est):
        for c in range(n_cls):
            ...
            for _ in range(20):                          # max tree depth
